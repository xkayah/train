package com.mnus.generator.utils;
...
public class DBUtil {
     ...
     * @throws Exception
     */
    public static List<FieldDB> getColumnByTableName(String tableName) throws Exception {
        List<FieldDB> fieldList = new ArrayList<>();
        Connection conn = getConnection();
        Statement stmt = conn.createStatement();
        ...
        stmt.close();
        conn.close();
        System.out.println(String.format("[%s] columns info:%s\n", tableName, JSONUtil.toJsonPrettyStr(fieldList)));
        return fieldList;
    }
